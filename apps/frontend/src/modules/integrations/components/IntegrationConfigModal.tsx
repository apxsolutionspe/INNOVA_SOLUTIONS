    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4">
      <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
        ...
        <div className="mt-4 grid gap-2 rounded-lg bg-slate-50 p-3 text-sm">
          {entries.length ? entries.map(([key, value]) => <div key={key} className="flex justify-between gap-3"><span className="font-semibold text-slate-600">{key}</span><span className="truncate text-slate-500">{String(value ?? 'No configurado')}</span></div>) : <p className="text-slate-500">Sin configuracion publica.</p>}
        </div>
