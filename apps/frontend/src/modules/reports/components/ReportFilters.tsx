...
export function ReportFilters({ filters, onChange, onRefresh }: Props) {
  const hasFilters = Boolean(filters.startDate || filters.endDate || filters.paymentMethod || filters.status);
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <p className="text-xs font-semibold text-slate-400">Los cambios actualizan todos los bloques del modulo.</p>
      </div>

      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[170px_170px_190px_1fr_auto_auto] xl:items-center">
        <input
          type="date"
