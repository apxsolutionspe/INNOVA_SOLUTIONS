...
import { ExportReportModule, SalesReport } from '../types/report.types';
...
export function SalesReportSection({
  ...
  return (
    <ReportChartCard title="Reporte de ventas" action={<ExportButtons module="sales" isExporting={isExporting} onExport={onExport} />}>
      <div className="grid gap-4 lg:grid-cols-[1.5fr_0.8fr]">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
          <ReportMetricCard label="Ventas registradas" value={report?.salesCount ?? 0} />
          ...
          <ReportMetricCard label="Ventas anuladas" value={report?.cancelledSales ?? 0} />
        </div>
      </div>
      ...
      <div className="mt-5">
        <ReportDataTable
          ...
          rows={(report?.rows ?? []).map((row) => [
            ...
            row.code,
            row.customer,
            row.paymentMethod.split(', ').map(formatPaymentMethod).join(', '),
