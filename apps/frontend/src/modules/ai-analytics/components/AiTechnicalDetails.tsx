  ...
  return (
    <details className="group rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm shadow-sm">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <Detail label="Caché" value={metadata.cached === true ? 'Sí' : metadata.cached === false ? 'No' : 'N/D'} />
        <Detail label="Fallback" value={metadata.fallbackUsed === true ? 'Sí' : metadata.fallbackUsed === false ? 'No' : 'N/D'} />
      </div>
